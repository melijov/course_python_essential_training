...
class bwDB:
  def __init__(self,**kwargs):
    """
      ...
      constructor method
        ...
        filename is for connecting to the dabase file   
    """
  ...
  def sql_do(self,sql,params=()):
    """
      ...
      method for non-select queries
        ...
        params is list of containing parameters
      returns nothing
    """
  ...
  def sql_query_row(self, sql, params=()):
    """
      db.sql_queryProw(sql[, params] )
      query for a single row
        sql is string containin SQL
        params is list containing parameters
      returns a single row as a Row factor
    """
    c = self._db.execute(sql, params)
    return c.fetchone()
  ...
  def sql_query_value(self, sql, params=()):
    ...
    """
    c = self._db.execute(sql,params)
    return c.fetchone()[0]  # returns first column from the first row
  ...
  def insert_nocommit(self, rec):
    ...
    q = 'INSERT INTO {} ({}) VALUES ({})'.format(self._table,', '.join(klist), ', '.join('?' * len(values)) )
  ...
  def update_nocommit(self, recid, rec):
    ...
    q = 'UPDATE {} SET {} WHERE id = ?'.format(self._table, ', '.join(map(lambda s: '{} = ?'.format(s), klist)) )
  ...
  def _filename(self, fn):
    self._dbfilename = fn
    self._db = sqlite3.connect(fn)  # setting the db connection
...
def test():
  ...
  print(f'Create database in {fn} ...',end='')
