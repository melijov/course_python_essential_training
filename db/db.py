...
def list_pagebar(pageno, numpages):
  ...
  nextlink = '<span class="n">&gt;%gt;</span>'
  linkback = g['linkback']
  
  if pageno > 0:
    prevlink = '<a href="{}?pageno={}&prevpage=1">&lt;%lt;</a>'.format(linkback, pageno)
  if pageno < (numpages - 1):
    nextlink = '<a href="{}?pageno={}&nextpage=1">&gt;%gt;</a>'.format(linkback, pageno)
...
def add():
  ...
  rec = dict(
    ...
    targetURL = cgi.entity_enconde(tURL)
  )
...
def error(*list, **kwargs):
  if 'cgi' in g:
    stackmessages('errors', *list, **kwargs)
  else:
    errorexit(' '.join(list))
...
def shorten(s):
  ...
  lsz = len(lookup)
  m = md5(s.encode('utf-8')) # md5 because it's short - doesn't meed to be secure or reversible
  out = m.digest();
