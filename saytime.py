...
class saytime(numwords):
  '''
  return the time (from two parameters) as workds,
  e.g. fourteen til noon, quarter past one, etc  
  '''
  
  _specials = {'noon': 'noon','midnight':'midnight', 'til':'til', 'past':'past'}
  ...
  def words(self):
    ...
    if m == 0:
      if h in (0,12): return hword    #for noon or midnight
